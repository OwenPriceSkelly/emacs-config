...
#+END_SRC
- Notes:
  I go back and forth on eglot/lsp-mode, usually for performance reasons. The highlighting on hover can be too slow sometimes to keep up with some movements and isn't great. If it doesn't noticably hurt performance, leaving it enabled and reducing ~eglot-send-changes-idle-time~ to 0 is good, otherwise just disable it. Everything else is solid.
** Plaintext
*** org
**** package: org (builtin)
